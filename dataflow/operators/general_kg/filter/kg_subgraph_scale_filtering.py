...
class KGSubgraphScaleFilter(OperatorABC):
    """
    ...
    def get_desc(lang: str = "en") -> tuple:
        if lang == "zh":
            return (
                "KGSubgraphScaleFilter 根据规模指标（如密度）筛选子图。",
                "读取子图数据及对应的规模得分列，保留得分在指定范围内的行。",
                "输入列 subgraph 为子图三元组列表，density（可配置）为规模得分列；过滤后直接写回 DataFrame，不新增输出列。",
            )
        return (
            "KGSubgraphScaleFilter filters subgraphs based on scale metrics such as density.",
            "Reads subgraph data and their scale score column, retaining only rows within the specified score range.",
