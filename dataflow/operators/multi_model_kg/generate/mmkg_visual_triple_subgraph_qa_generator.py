...
from dataflow.prompts.diverse_kg.mmkg import MMKGSubgraphBaseQAGenerationPrompt


@OPERATOR_REGISTRY.register()
@prompt_restrict(MMKGSubgraphBaseQAGenerationPrompt)
class MMKGSubgraphBaseQAGeneration(OperatorABC):
    ...
    def run(
        ...
        for _, row in tqdm(df.iterrows(), total=len(df), desc="Generating QA"):
            ...
            if isinstance(subgraph, str):
                subgraph = [s.strip() for s in subgraph.split("\n")]

            result = self.process_single_record(subgraph, vis_triple_list, img_dict)
            all_qas.append(result["QA_pairs"])

        df[output_key] = all_qas
