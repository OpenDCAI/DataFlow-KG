from dataflow.prompts.diverse_kg.medkg import MedKGDrugActionMechanismPrompt
import pandas as pd
...
@prompt_restrict(
    MedKGDrugActionMechanismPrompt,
)
@OPERATOR_REGISTRY.register()
class MedKGTripleDrugActionMechanismDiscovery(OperatorABC):
    ...
    def __init__(
        ...
        self.prompt_template = (
            ...
            else MedKGDrugActionMechanismPrompt(lang=self.lang)
        )
    ...
    def get_desc(lang: str = "en"):
        if lang == "zh":
            return (
                "MedKGTripleDrugActionMechanismDiscovery 用于根据查询和知识图谱候选路径生成药物作用机制解释",
                "输入: query, triple; 输出: mechanism_path, mechanism_answer"
            )
        else:
            return (
                "MedKGTripleDrugActionMechanismDiscovery generates drug action mechanism explanations from query and KG paths.",
                "Input: query, triple; Output: mechanism_path, mechanism_answer"
            )
    ...
    def _parse_llm_response(self, response: str) -> Dict[str, Any]:
        try:
            ...
            mechanism_path = data.get("mechanism_path", [])
            mechanism_answer = data.get("mechanism_answer", "")

            if not isinstance(mechanism_path, list):
                mechanism_path = []
            if not isinstance(mechanism_answer, str):
                mechanism_answer = str(mechanism_answer)
    ...
    def run(
        ...
        for query, triples in zip(queries, triples_list):
            ...
            mechanism_paths.append(result.get("mechanism_path", []))
            mechanism_answers.append(result.get("mechanism_answer", ""))

        dataframe[self.output_key_path] = mechanism_paths
        dataframe[self.output_key_answer] = mechanism_answers

        output_file = storage.write(dataframe)
        self.logger.info(f"Drug action mechanism discovery saved to {output_file}")
