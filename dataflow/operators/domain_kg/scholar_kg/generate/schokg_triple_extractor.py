from dataflow.prompts.diverse_kg.schokg import SchoKGRelationExtractorPrompt
import pandas as pd
...
@prompt_restrict(
    SchoKGRelationExtractorPrompt,
)
@OPERATOR_REGISTRY.register()
class SchoKGTripleExtraction(OperatorABC):
    r"""
    ...
    def __init__(
        ...
        self.prompt_template = (
                SchoKGRelationExtractorPrompt(lang=self.lang)
            )        
    ...
    def get_desc(lang: str = "en") -> tuple:
        if lang == "zh":
            return (
                "SchoKGTripleExtraction 是一个三元组抽取算子，用于从文本中抽取知识图谱三元组。",
                "输入为原始文本及其对应的合法实体列表，输出为结构化的三元组结果。"
            )
        else:
            return (
                "SchoKGTripleExtraction extracts triples from text.",
    ...
    def _load_single_ontology_from_cache(
        ...
        ontology_data = storage_meta.read(
            file_path=f"./.cache/schokg/{ontology_file_name}",
