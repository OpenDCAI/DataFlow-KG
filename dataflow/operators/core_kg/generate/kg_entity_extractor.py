...
from dataflow.prompts.core_kg.rel_triple_generate import KGEntityExtractionPrompt
import pandas as pd
...
@prompt_restrict(KGEntityExtractionPrompt)
@OPERATOR_REGISTRY.register()
class KGEntityExtraction(OperatorABC):
    """
    ...
    def __init__(
        ...
        self.prompt_template = (
            prompt_template
            if prompt_template
            else KGEntityExtractionPrompt(lang=self.lang)
        )
    ...
    def process_batch(
        ...
        for text, source in tqdm(
            ...
            results.append({"entity": entities})
    ...
    def run(
        ...
        dataframe[self.output_key] = [o["entity"] for o in outputs]
