...
from dataflow.prompts.core_kg.rel_triple_eval import KGQAConcisenessPrompt


class KGQAConcisenessEvaluator(OperatorABC):
    ...
    def __init__(
        ...
        self.prompt_manager = KGQAConcisenessPrompt(lang)
    ...
    def get_desc(lang: str = "en") -> tuple:
        if lang == "zh":
            return (
                "KGQAConcisenessEvaluator 用于评估知识图谱 QA 对的简洁性。",
                "使用 LLM 对 QA 对进行简洁性打分。",
                "输入列 QA_pairs 为 QA 问答对列表，输出列 conciseness_scores 为每对 QA 的简洁性评分列表。"
            )
        else:
            return (
                "KGQAConcisenessEvaluator evaluates the conciseness of KG-derived QA pairs.",
                "Uses an LLM to score each QA pair on conciseness.",
                "Takes QA_pairs (List of QA pairs) as input and outputs conciseness_scores (List of scores per QA pair)."
            )
