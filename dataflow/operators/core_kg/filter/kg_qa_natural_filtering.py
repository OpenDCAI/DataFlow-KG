...
class KGQANaturalFilter(OperatorABC):
    """
    ...
    def get_desc(lang: str = "en") -> tuple:
        if lang == "zh":
            return (
                "KGQANaturalFilter 根据自然度得分对 QA 对进行过滤。",
                "读取 QA 对及对应的自然度得分，保留得分在指定范围内的 QA 对。",
                "输入列 QA_pairs 为 QA 问答对列表，naturalness_scores 为对应得分列表；输出列 filtered_QA_pairs 为过滤后的 QA 对列表。",
            )
        return (
            "KGQANaturalFilter filters QA pairs based on naturalness scores.",
            "Reads QA pairs and their naturalness scores, retaining only those within the specified score range.",
            "Takes QA_pairs (List of QA pairs) and naturalness_scores (List of float) as inputs, and outputs filtered_QA_pairs (List of QA pairs that pass the score threshold).",
        )
    ...
    def run(
        ...
        score_key: str = "naturalness_scores",
