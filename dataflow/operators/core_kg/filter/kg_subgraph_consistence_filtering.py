...
class KGSubgraphConsistenceFilter(OperatorABC):
    """
    ...
    def get_desc(lang: str = "en") -> tuple:
        if lang == "zh":
            return (
                "KGSubgraphConsistenceFilter 根据一致性得分筛选子图。",
                "读取子图数据及对应的一致性得分列，保留得分在指定范围内的行。",
                "输入列 subgraph 为子图三元组列表，consistency_score（可配置）为一致性得分列；过滤后直接写回 DataFrame，不新增输出列。",
            )
        return (
            "KGSubgraphConsistenceFilter filters subgraphs based on consistency scores.",
            "Reads subgraph data and their consistency score column, retaining only rows within the specified score range.",
            "Takes subgraph (List[str]) and a score column (default: consistency_score) as inputs; filtered rows are written back to the DataFrame without adding new columns.",
        )
